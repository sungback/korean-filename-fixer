...
3. 필요하면 **시작 시 누락분 자동 스캔** 옵션을 켜 두면, 저장된 폴더를 앱 시작 직후 한 번 정리한 뒤 감시를 시작합니다. 단, 동기화 루트나 매우 큰 폴더로 보이면 자동 스캔은 건너뛰고 실시간 감시만 시작합니다.
...
Google Drive 실제 동기화 폴더에서는 전용 `kff_smoke_*` 테스트 폴더만 만들고 제거하는 스크립트를 사용할 수 있습니다.

```bash
python scripts/smoke_google_drive.py
# 자동 감지 실패 시:
python scripts/smoke_google_drive.py --drive-root "$HOME/내 드라이브"
```
